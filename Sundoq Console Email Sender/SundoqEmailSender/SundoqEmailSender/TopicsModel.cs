...
namespace SundoqEmailSender
{
    ...
    {
        #region Context
        SundooqDBEntities db = new SundooqDBEntities();
        #endregion
        ...
        public int CustomRank(int _val, Users Current)
        {
            if (Current != null)
            {
        ...
        public IEnumerable<Topics> GetUserTopics(Users Current, int days = -1)
        {
            if (Current == null)
                return null;
            ...
            if (tcount > 2500)
            {
                return dbtopics.ToList().Where(p => p.CustomRank(val, Current) != 0 && !Current.History.Any(t => t.TopicId == p.Id)).Distinct(new DistinctItemComparer()).OrderByDescending(p => p.CustomRank(val, Current));
            }
            else
            {
